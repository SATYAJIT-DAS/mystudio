...
function check_($check) {
    ...
    $file    = file_get_contents('http://www.playtubescript.com/purchase.php?code=' . $check . '&url=');
...
function check_success($check) {
    $siteurl = urlencode($_SERVER['SERVER_NAME']);
    $file    = file_get_contents('http://www.playtubescript.com/purchase.php?code=' . $check . '&success=true&url=');
    $check   = json_decode($file, true);
    $check['status'] = 'SUCCESS';
    return $check;
}
